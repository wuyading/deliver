 ...
 *
 * See [[\Zilf\Db\ActiveRecord]] for a concrete implementation.
 *
...
abstract class BaseActiveRecord extends Model implements ActiveRecordInterface
{
     ...
     * @throws \Zilf\Db\Exception\InvalidParamException if relation name is wrong
     ...
     *                       the attributes of the record associated with the `$class` model,
     *                       while the values of the array refer to the corresponding
     *                       attributes in **this** AR class.
     ...
     *                           identical values using `===`, defaults to `true`.
     *                           Otherwise `==` is used for comparison. This parameter
     ...
     *
     * @param bool  $insert            whether this method called while inserting a record.
     *                                 If `false`, it means the method is called while
     *                                 updating a record.
    ...
    {
        $this->trigger(
            $insert ? self::EVENT_AFTER_INSERT : self::EVENT_AFTER_UPDATE, new AfterSaveEvent(
                [
     ...
     *                                             This parameter is only meaningful for a relationship
     *                                             involving a junction table (i.e., a relation set with
     *                                             [[ActiveRelationTrait::via()]] or
     ...
     *                                       You have to make sure that the model is really
     *                                       related with the current model as this method
     *                                       does not check this.
     * @param  bool                  $delete whether to delete the model that contains the foreign key.
     *                                       If `false`, the model's foreign key will be set `null` and
     *                                       saved. If `true`, the model containing the foreign key
     *                                       will be deleted.
