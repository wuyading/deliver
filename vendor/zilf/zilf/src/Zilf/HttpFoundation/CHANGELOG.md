 ...
 * Improved error-handling of `\Zilf\HttpFoundation\Session\Storage\Handler\PdoSessionHandler`
 ...
 * [BC BREAK] Moved all session related classes and interfaces into own namespace, as
   `Zilf\HttpFoundation\Session` and renamed classes accordingly.
   Session handlers are located in the subnamespace `Zilf\HttpFoundation\Session\Handler`.
 * SessionHandlers must implement `\SessionHandlerInterface` or extend from the
   `Zilf\HttpFoundation\Storage\Handler\NativeSessionHandler` base class.
 ...
 * Added `Zilf\HttpFoundation\Attribute\AttributeBag` to replicate
   attributes storage behaviour from 2.0.x (default).
 * Added `Zilf\HttpFoundation\Attribute\NamespacedAttributeBag` for
