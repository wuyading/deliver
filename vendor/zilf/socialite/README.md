...
You can set the request with your custom `Request` instance which instanceof `Zilf\HttpFoundation\Request` before you call `driver` method.
...
By default, the `SocialiteManager` use `Zilf\HttpFoundation\Session\Session` instance as session manager, you can change it as following lines:
...
> Your custom session manager must be implement the [`Zilf\HttpFoundation\Session\SessionInterface`](http://api.symfony.com/3.0/Symfony/Component/HttpFoundation/Session/SessionInterface.html).
